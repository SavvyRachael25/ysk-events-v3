...
for (const v of ["v1", "v2", "v3"]) {
  ...
  const m = await b.newPage(); await m.setViewport({ width: 390, height: 844, deviceScaleFactor: 2 });
  await m.goto(`http://localhost:3077/mock/${v}`, { waitUntil: "networkidle0" }); await new Promise(r => setTimeout(r, 1200));
  await m.screenshot({ path: `${S}/mock-${v}-m.png` }); await m.close();
  await p.close();
}
await b.close();
